...
int activation_shape[3] = {384, SIZE, SIZE};
...
void convolution(float ****output, float ****input, int layer, int activation_size, int batch_size, int stride)
    {
                    ...
                    for (m = 0; m < activation_size; m = m + stride)
                        ...
                        for (e = 0; e < weights_shape[layer][2]; e++)
                            ...
                            for(r = 0; r < weights_shape[layer][3]; r++)
                                {
                                sum += input[w][j][k + e][m + r] * conv[layer][i][j][e][r];
                                }
                             }
                        output[w][i][k][m] = output[w][i][k][m] + sum;
                        }
                    }
                add_bias_and_relu(output[w][i], activation_size);
                }
            }
        }
    }
